...
**📚 Prerequisites**: [🎨 C-IDE: Inclusive design 101](C-IDE.md) TODO: link to assistive technologies desktop
...
### Keyboard navigation (what are supposed to do each key)

Keyboard navigation can be a bit tricky. Most resources you can find on accessibility, only tell you that elements should be usable with a keyboard but not what combination of keys is intuitive to navigate it. In reality there are 5 main type of keyboard inputs. Implementing those inputs properly would make your application usable with a keyboard.

#### Tab & Backtab

The Tab and backtab (Shift+Tab) keys are the main way of navigating trough the UI. Pressing it should respectively send the focus to the next or previous focusable element of the UI. It should not be used to navigate trough the elements of a list or go onto static text. The static text should still be accessible in the label of an associated element.
...
The escape key is supposed to close popups and dropdowns. Having it broken is a common problem in many applications. This is what we call a "focus trap" where an element take focus without any way with the keyboard to make it release it. Having the escape key working properly is absolutely necessary in order to make your application keyboard-accessible.

#### Alt

The alt key has two main purposes. Firstly it should close and escape fields where tab inputs would be absorbed (some text inputs for example). Secondly it should focus the menu attached to the top of an application. Here is an example on VScode.
...
Your application is now usable with a keyboard, but you shouldn't be content with usability. You should be designing for you app to be fun and fluid, not "usable". Keyboard shortcuts are the key to achieve that. They make it unnecessary to circle though all elements with Tab inputs. Indeed, a user used to the shortcuts of an application can operate it much faster than one with a mouse.
